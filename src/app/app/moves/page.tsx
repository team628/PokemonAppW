...
export default async function MovesPage({
  ...
  const insights = await buildInsights(user.id, Number.isFinite(budget as number) ? budget : null);
    ...
    <>
      ...
      <main className="px-4 pb-8 pt-4 lg:px-0">
        <form className="panel mb-4 flex items-end gap-2 px-3.5 py-3 lg:max-w-sm" action="/app/moves">
          <div className="flex-1">
            ...
            <input id="budget" name="budget" type="number" inputMode="decimal" step="1" min="0"
              defaultValue={budget ? (budget / 100).toString() : ''} placeholder="50"
        ...
        <SourceNote className="mt-6 border-t border-ink-line pt-4">
          ...
          quoted on a stated date. Costs are card prices only and exclude postage.
          {insights.historyTooShallow &&
            ' Price-movement moves are withheld: SetValue holds a single dated reading per card, and one point is not a trend.'}
        </SourceNote>
      </main>
    </>
  );
