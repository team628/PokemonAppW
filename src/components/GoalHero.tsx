 ...
 * from.
 */
export function GoalHero({ view }: { view: GoalView }) {
  const { goal, set, metrics: m } = view;
  const modeLabel = GOAL_MODES.find((x) => x.id === goal.mode)?.label ?? goal.mode;
  ...
  return (
    <section className="panel relative overflow-hidden px-5 pb-5 pt-5">
      ...
      <div className="flex items-baseline justify-between gap-3">
        <Link href={`/app/sets/${set.id}?mode=${goal.mode}`} className="min-w-0">
          <p className="truncate text-sm font-bold">{set.name}</p>
          <p className="text-[11px] uppercase tracking-[.14em] text-ink-mute">
            {modeLabel} · {set.series}
          </p>
        </Link>
        {goal.pinned === 1 && <span className="chip shrink-0 text-gold">Pinned</span>}
      </div>
      ...
      {done ? (
        <div className="mt-5">
          ...
          <p className="mt-1 text-xs text-ink-mute">
            ...
            {goal.completed_at ? new Date(goal.completed_at).toLocaleDateString() : 'recently'}.
          </p>
        </div>
      ) : (
        <div className="mt-5">
      ...
      {!done && (
        <div className="mt-3 flex gap-2">
          <Link href={`/app/sets/${set.id}?mode=${goal.mode}&filter=missing`} className="btn-need flex-1">
            See what&apos;s missing
          </Link>
          <Link
            href={`/app/show?set=${set.id}`}
      ...
      <SourceNote className="mt-3 border-t border-ink-line pt-3">
        ...
        {m.oldestObservation && <>Oldest reading {m.oldestObservation}.</>}
