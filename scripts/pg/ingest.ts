/**
 * Loads the catalog and prices into PostgreSQL through the provider abstraction.
 *
 * Runs as service_role — the only role permitted to write the catalog. Uses
 * COPY-style batched inserts rather than row-at-a-time round trips, because the
 * full catalog is ~20k cards and ~35k printings.
 *
...
import { activeProvider } from '../../src/lib/providers/pokemontcg';
import type { ProviderSet } from '../../src/lib/providers/types';
...
async function ingestCatalog() {
  const sets: ProviderSet[] = await provider.listSets();
